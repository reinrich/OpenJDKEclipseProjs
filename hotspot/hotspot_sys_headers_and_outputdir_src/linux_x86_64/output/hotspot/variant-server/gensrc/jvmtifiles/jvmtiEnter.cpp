...
bool JvmtiUtil::has_event_capability(jvmtiEvent event_type, const jvmtiCapabilities* capabilities_ptr) {
  switch (event_type) {
    ...
    case JVMTI_EVENT_SAMPLED_OBJECT_ALLOC:
      return capabilities_ptr->can_generate_sampled_object_alloc_events != 0;
...
static jvmtiError JNICALL
jvmti_ClearAllFramePops(jvmtiEnv* env,
            jthread thread) {
  ...
  MACOS_AARCH64_ONLY(ThreadWXEnable __wx(WXWrite, current_thread));
  ThreadInVMfromNative __tiv(current_thread);
  VM_ENTRY_BASE(jvmtiError, jvmti_ClearAllFramePops , current_thread)
  DEBUG_ONLY(VMNativeEntryWrapper __vew;)
  PreserveExceptionMark __em(this_thread);
