...
#
# Given the output directory of an OpenJDK build this script creates settings.xml and a zip archive
# with referenced system headers and referenced outputdir sources for a build platform which can be
# imported in the cdt project as a build configuration.
# 
...
if [ $# -ne 4 ] ; then
    ...
    echo
    echo "usage: [DEBUG_MODE=1] `basename $0` <build-host> <gnu-sed> <tmp dir> <outputdir>"
    echo
    exit 1
fi
...
function assert_file_exists() {
    ff="$1"
    if [ ! -f "$ff" ] ; then report_and_exit "file '$ff' does not exist." 2; fi
}
...
function search_file_going_up() {
    ...
    while [ ! -f "$RES/${FILE}" ] ; do
        RES="$(readlink -f "$RES/..")"
        if [ "$RES" = "/" ] ; then
            report_and_exit "${FILE} was not found in a parent or grandparent directory of $RES"
        fi
    done
    echo "$RES"
}
...
function initialize() {
    CDT_PROJECT_NAME="jdk_native"

    # find OUTPUTDIR looking for spec.gmk
    echo "OUTPUTDIR is $OUTPUTDIR"
    if [ ! -f "$OUTPUTDIR/spec.gmk" ] ; then report_and_exit "spec.gmk in given OUTPUTDIR=$OUTPUTDIR not found" ; fi

    mkdir -p $WORK_DIR
    cmdline_file=$OUTPUTDIR/support/native/java.base/libjava/ClassLoader.*.cmdline
    assert_file_exists $cmdline_file
    source_file=$($GSED 's/.*\.o \(.\+.c\)$/\1/' <$cmdline_file)
    assert_file_exists $source_file

    JDK_DIR_ABS=$(dirname $source_file)
    JDK_DIR_ABS="$(search_file_going_up LICENSE "$JDK_DIR_ABS")"
    ...
    SYS_HEADERS_AND_OUTPUTDIR_SRC_ARCHIVE_TOP_DIR=sys_headers_and_outputdir_src
    SYS_HEADERS_AND_OUTPUTDIR_SRC_ARCHIVE_STEM_DIR=$SYS_HEADERS_AND_OUTPUTDIR_SRC_ARCHIVE_TOP_DIR/${OS}_${CPU}_${BITS} # e.g. sys_headers_and_outputdir_src/windows_x86_64
    SYS_HEADERS_AND_OUTPUTDIR_SRC_ARCHIVE_STEM_DIR_ABS=$WORK_DIR/$SYS_HEADERS_AND_OUTPUTDIR_SRC_ARCHIVE_STEM_DIR # e.g. /tmp/cdt/work/sys_headers_and_outputdir_src/windows_x86_64
    SYS_HEADERS_ARCHIVE_DIR=sys_headers
    SYS_HEADERS_ARCHIVE_DIR_ABS=$SYS_HEADERS_AND_OUTPUTDIR_SRC_ARCHIVE_STEM_DIR_ABS/$SYS_HEADERS_ARCHIVE_DIR # e.g. /tmp/cdt/work/sys_headers_and_outputdir_src/windows_x86_64/sys_headers
    OUTPUTDIR_IN_ARCHIVE_DIR=output
    OUTPUTDIR_IN_ARCHIVE_DIR_ABS=$SYS_HEADERS_AND_OUTPUTDIR_SRC_ARCHIVE_STEM_DIR_ABS/$OUTPUTDIR_IN_ARCHIVE_DIR # e.g. /tmp/cdt/work/sys_headers_and_outputdir_src/windows_x86_64/gensrc
...
function extract_quoted_macro_definitions() {
    ...
    # Special case 1: -DJAVA_ARGS='{ "-J-ms8m", "-m", "java.base/sun.security.tools.keytool.Main", }'
    SPEC1="s/.*-DJAVA_ARGS='\([^']*\)'.*/JAVA_ARGS=\1/"
    DEL1="s/-DJAVA_ARGS='\([^']*\)'//"
    ...
    $GSED "$SPEC1" $file_with_definitions | tee -a ${ALL_DEFINES}.spec
    $GSED "$DEL1 ; s/-D */\n-D/g; $V4_2_V7 ; $V6_2_V1 ; $V7_2_V5 ; $V3_2_V5 ; $V5 ; $V1 ; $V2" $file_with_definitions \
        | $GSED -n '/^-D/s///p' | $GSED -n '/THIS_FILE=/!p' | tee ${ALL_DEFINES}.inc
}
...
function add_include_dirs_using_cxx_cmdline() {
    ...
    case $OS in
        linux|bsd)
            ...
            $GSED 's%\(.*\) -c .* \([^ ]\+\.c\)%\1 -E -Wp,-v \2%' <$cmdline_file >$cmdline_file_mod
        ...
        solaris)
            ...
            # modify cmdline to let the compiler print list of dirs searched for includes
            $GSED 's%\(.*\) -c .* \([^ ]\+\.c\)%\1 -verbose=diags \2%' <$cmdline_file >$cmdline_file_mod
...
function add_cxx_compiler_predifined_macros_and_include_dirs() {
    ...
    in_cmdline=`$READLINK -f $OUTPUTDIR/support/native/jdk.jdwp.agent/libjdwp/exec_md.*.cmdline`
    echo "using $in_cmdline"

    if [ ! -e $in_cmdline ] ; then
        report_and_exit "$in_cmdline missing"
    fi
    
    cxx_predefines_cmdline_file=$WORK_DIR/cxx_predefines.cmdline
    predefines_file=$WORK_DIR/cxx_predefines.c
    empty_c=$WORK_DIR/empty.c
    del_contents $empty_c
    ...
    case $OS in
        linux|bsd|solaris)
            ...
            if [ $OS = solaris ] ; then
                $GSED "s!-c -xMMD \(-xMP \|\)-xMF .*!-E -xdumpmacros $empty_c!" <$in_cmdline >$cxx_predefines_cmdline_file
            else
                $GSED "s!-c -MMD \(-MP \|\)-MF .*\$!-E -dM -o $predefines_file $empty_c!" <$in_cmdline >$cxx_predefines_cmdline_file
            fi
        ...
        aix)
            ...
            $GSED "s!-c -qmakedep=gcc .*!-E -qshowmacros=pre -V $empty_cpp!" <$in_cmdline >$cxx_predefines_cmdline_file
        ...
        windows)
            ...
            $GSED "s!$CMDLINE_PATTERN_TO_REPLACE!-E -Bxread_predefs_from_env.cmd $empty_c!" <$in_cmdline >$cxx_predefines_cmdline_file
...
function generate_proj_settings {
    ...
    MAP_JDK_SRC="\%$JDK_DIR_ABS/src/\([^/]*\)% s%%!$CDT_PROJECT_NAME/\1_src%"

    MAP_GENSRC="\%$OUTPUTDIR% s%%!$CDT_PROJECT_NAME/sys_headers_and_outputdir_src/${OS}_${CPU}_${BITS}/${OUTPUTDIR_IN_ARCHIVE_DIR}%"
    MAP_SYSINCLS="s%^/%!$CDT_PROJECT_NAME/sys_headers_and_outputdir_src/${OS}_${CPU}_${BITS}/${SYS_HEADERS_ARCHIVE_DIR}/%"
...
EOF
$GSED "$MAP_HS_SRC ; $MAP_JBASE_SRC ; $MAP_JDK_SRC ; $MAP_J2SE ; $MAP_LIBS ; $MAP_GENSRC ; $MAP_SYSINCLS ; s%!%/% ; $XML_DECO_INCLUDE" <$ALL_INCLUDE_DIRS >>$SETTINGS_XML
cat >> $SETTINGS_XML <<EOF    
...
#    jdk.jdwp.agent/libjdwp/ArrayReferenceImpl.
#    jdk.jdwp.agent/libdt_socket/socketTransport.
#    java.base/libjava/ClassLoader.
#    java.base/keytool/main.
)
CMDLINE_FILE_PATTERNS=${CMDLINE_FILE_PATTERNS[*]}

if [ "$CMDLINE_FILE_PATTERNS" = "" ] ; then
    # we could scan all cmdline files from all libs
    CMDLINE_FILE_PATTERNS='jdk.jdwp.agent/*/*. java.base/*/*.'
fi

# Add prefix $OUTPUTDIR/support/native/ and suffix *.cmdline to each pattern given in CMDLINE_FILE_PATTERNS
# (example $OUTPUTDIR/support/native/jdk.jdwp.agent/libjdwp/exec_md.o.cmdline)
CMDLINE_FILE_PATTERNS=$(
    set -f # no globbing
    for pattern in $CMDLINE_FILE_PATTERNS ; do
        echo -n "$OUTPUTDIR/support/native/$pattern*.cmdline "
    done
)
...
for cur_cmdline_file in $CMDLINE_FILES ; do
    ...
    if ! $GSED -n '/\.c$/! q 1' <$cur_cmdline_file ; then
        echo "no C compilation - skipping."
        continue
    fi
